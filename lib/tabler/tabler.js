    ...
    **/
    ...
    _.extend(Tabler.prototype, {
        ...
        **/
        ...
        concatTagStrings: function(builder, tag, text) {
        ...
        **/
        makeTag: function(tag, text, attrs){
            ...
            return this.concatTagStrings(builder, tag, text);
        },
        /*
        ...
        **/
        render: function(){
            ...
            return this.fetch(options, _(this.renderTable).bind(this));
        },

        updateCells: function($tds, cellIndex, cellMarkup) {
            $tds.eq(cellIndex).replaceWith(cellMarkup);
        },
        ...
        update: function(rowIndex, newItem, options){
            ...
            _(spec).forEach(function(spec, cellIndex){
                ...
                if(_.intersection(updateFieldNames, fields).length > 0){
                    this.updateCells($tds, cellIndex, this.renderCell(newItem, spec, rowIndex));
                }
            }, this);
        },

        renderTableSections: function (tableSections) {
            var head = tableSections.length > 0 && tableSections[0];
            var body = tableSections.length > 1 && tableSections[1];
            var foot = tableSections.length > 2 && tableSections[2];
        ...
        renderTable: function(data){
            ...
            this.renderTableSections([head, body, foot]);
        },
        ...
        **/
        renderBody: function(data, spec){
            ...
            return this.joinBodyStrings(body)
        },

        joinRowCells: function(row, cells) {
            return [this.renderBodyTr(row)].concat(cells).concat('</tr>').join('\n');
        },

        renderRow: function(row, i, spec){
            var self = this;
            var cells = _(spec).map(function(colSpec) {
                return self.renderCell(row, _.clone(colSpec), i);
            });

            return this.joinRowCells(row, cells);
        },
